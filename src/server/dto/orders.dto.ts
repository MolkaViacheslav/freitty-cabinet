...
export function mapOrderDetail(order: OrderDetailRow) {
  ...
  return {
    ...
    // Same rule as suppliesSubtotal (DECISIONS.md B9): totals are computed once, on the way out,
    // so no component has to add anything up for itself.
    subOrdersPallets: order.subOrders.reduce((sum, so) => sum + so.pallets, 0),
    operations,
    supplies,
