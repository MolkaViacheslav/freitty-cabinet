...
**Поточний етап: 7 (Поліш закрито й задеплоєно — усі заплановані етапи виконано)** ← оновлювати в кінці кожної сесії
...
- [x] Прогнати seed на проді — спитав перед виконанням (змінює дані на живій базі), користувач
      підтвердив, прогнано, 16/16 assertion-ів зелені (див. Session Log нижче)
...
- `2026-09-04 17:30` — **Alert-чіп на дашборді вів не туди (знайшов користувач).** Чіп «2 · alert»
  клікався на `/orders?search=FR001674` — один ордер замість двох, бо `orderNumber` у бакеті це
  представник для тексту, а не адреса бакета. Тепер alert-чіп веде на `?tab=alerts` (таб фільтрує
  рівно цей бакет — перевірено: 2 ордери, `FR001674` + `FR002000`); `awaiting-action` не чіпав,
  його `?search=` точний, поки count = 1 (таба/фільтра під цей прапорець немає — окремо
  зазначено в коментарі й контракті). Щоб не матчитись на `label` (текст для показу), бакет
  дістав стабільне поле `kind: "alert" | "awaiting-action"` у `dashboard.service.ts`;
  `api-contract.md` оновлено разом із кодом, включно з явною приміткою, що `orderNumber`/`detail`
  — приклад, а не вміст бакета. `tsc`/`eslint`/`build`/81 тести — чисті.
  / Далі: за користувачем. / Відкрито: без змін.
- `2026-09-04 16:05` — **Закриття двох відкритих питань з попереднього запису + видалення Етапу
  8 з плану за прямим проханням користувача.**
  - Seed прогнано на проді (користувач підтвердив після явного запитання): 16/16 assertion-ів
    зелені (72 ордери, 7 active, 24 completed30d, 3 needAttention — ті самі контрольні числа, що
    й завжди). Перевірено на живому URL після сіда: `/api/dashboard/summary` і `/orders`
    повертають свіжі дані без помилок.
  - Зміни Етапу 7 закомічено й запушено в `main` (користувач підтвердив), коміт `bf9a040`.
  - **Етап 8 «Презентація» прибрано з плану повністю** — користувач сказав, що презентації не
    буде, і попросив не лишати про неї жодних згадок у плані. Прибрано: секцію "Етап 8" і
    відповідний пункт у "Лінія відсічення" (PROGRESS.md), рядок `docs/presentation.md` з карти
    документів, ризик-секції (Supabase pause) і дерева репо (PROGRESS.md), рядок
    `docs/presentation.md` з таблиці "Read these before starting" (CLAUDE.md), а також фразу "This
    is the key point in the presentation" там же. `docs/DECISIONS.md` (B-секція) і
    `docs/data-model.md` (§2.5) втратили формулювання "на презентації озвучити" —
    змістовна частина (чому wireframe суперечить сам собі, три причини моделі) лишилась, тільки
    без прив'язки до презентації. Сам файл `docs/presentation.md` видалено — це був сценарій
    демо, без презентації він не потрібен ні як довідник, ні як план.
    **Свідомо НЕ чіпав** `docs/task.md` — це запис оригінального завдання ("оцінюють задеплоєний
    застосунок + презентацію"), а не наш план; це історичний факт умови, а не пункт, який ми самі
    вирішуємо виконувати. Якщо презентації дійсно не буде — це розбіжність з тим, що просив
    замовник тестового завдання, не з тим, що написано в `PROGRESS.md`/`CLAUDE.md`; вирішувати це
    користувачу, я лише прибрав згадки з **плану**, як прямо попросили.
    Історичні записи Session Log (нижче), включно з тими, що згадують `docs/presentation.md`
    (наприклад, W7-розслідування нижче), **не переписував** — це протокол того, що реально
    відбувалось у минулих сесіях, а не план.
  / Далі: план закрито; наступний крок за користувачем. / Відкрито: розбіжність
  `docs/task.md` (умова згадує презентацію) із рішенням користувача не робити презентацію —
  не мій виклик, лишаю як є.
- `2026-09-04 15:45` — **Етап 7 закрито й задеплоєно.** Поліш, доменну логіку/сервіси/схему не
  чіпав, як і просили.
  - **`loading.tsx`/`error.tsx`/`not-found.tsx`** виявились уже повністю готові з попередніх
    сесій (ревʼю Етапів 0–3 і 6) — перевірив живцем, нічого не додавав. Один пункт із запиту
    користувача свідомо **не** виконав буквально: "loading.tsx для Order Detail" суперечить
    прямому правилу CLAUDE.md ("No `loading.tsx` above `orders/[number]`") і задокументованій
    причині 404-фікса з Етапу 6 — Suspense-межа над сторінкою знову дала б 200 замість 404 на
    неіснуючому номері. Замість цього просто підтвердив на проді, що `notFound()` досі веде на
    `(cabinet)/not-found.tsx` з реальним HTTP 404 (`curl -D-` → `404`, решта сторінок → `200`).
  - **Адаптив.** `Sidebar.tsx` — єдиний новий `"use client"`-листок (component-conventions:
    інтерактивність виносити в найменший компонент, не в layout): бургер-кнопка `fixed
    top-3 left-3 md:hidden`, backdrop, `<aside>` як off-canvas drawer (`-translate-x-full` →
    `translate-x-0`, `md:translate-x-0` завжди на десктопі), клік по "Orders" закриває меню.
    `Topbar.tsx` — `pl-14 md:pl-6` резервує місце під бургер, декоративний search-бокс (`Out of
    scope`) ховається нижче `sm`, а не стискається в перенесений текст. `OrdersTabs.tsx` —
    `overflow-x-auto` замість переносу 5 табів на два рядки.
    **Знахідка, не з чекліста запиту:** `document.documentElement.scrollWidth` на 375px показав
    горизонтальний скрол на *всій сторінці* (`/orders?view=table` — 889px замість 375px), хоча
    `DataTable` вже мав власний `overflow-x-auto`. Причина — жоден flex-предок (`(cabinet)/
    layout.tsx`: колонка контенту + `<main>`) не мав `min-w-0`, тому auto-мінімальна ширина
    flex-айтема бралась з min-content широкої таблиці й розтягувала всю сторінку замість
    активації внутрішнього скролу. Додав `min-w-0` у двох місцях `layout.tsx` — після цього
    `overflow-x-auto` таблиці нарешті працює як задумано (скрол лишається всередині таблиці,
    сторінка — ні). Друга знахідка: `NeedAttentionCard`'s "Open list →" був `absolute
    right-3 bottom-2.5` — на 375px чіпи `breakdown` переносяться на другий рядок, картка не
    росте під абсолютний елемент (він поза потоком), і посилання лягало прямо на текст чіпа.
    Прибрав `absolute`, посилання тепер у звичайному потоці під чіпами. Обидві знахідки
    перевірені скріншотами Playwright (375×812) до/після — без них не було б видно, зверху
    "не ідеал" виглядало б як "працює".
    Перевірено на **проді**, не тільки локально: Dashboard/Order List (Cards і Table)/Order
    Detail — `scrollWidth === clientWidth` на всіх чотирьох, бургер відкриває drawer
    (`aside` box `{x:0,y:0,width:220,height:812}`), клік по "Orders" у drawer навігує і закриває
    його. Десктопний вигляд (`md:` і вище) не чіпав — жоден клас там не змінився.
  - **`README.md`** — стек, локальний запуск (env, дві URL Supabase з поясненням, чому вони не
    взаємозамінні, `migrate dev` → `db seed` → `dev`), список команд, посилання на прод і на
    `docs/DECISIONS.md`. Не дублює `CLAUDE.md` (це файл для людей, не для Claude Code).
  - **Прод-верифікація "інкогніто" (Playwright, свіжий `BrowserContext` без cookies/storage —
    функціональний еквівалент інкогніто, не сам Chrome):** Dashboard → `?tab=alerts` на Order
    List (2 рядки, збігається з лічильником) → Order Detail конкретного ордера → неіснуючий
    `FR999999` (HTTP 404, текст "Order not found" є в HTML) — жодного білого екрана,
    `console.error`/`pageerror` під час проходу відсутні (крім очікуваного 404 на самому
    неіснуючому запиті).
  - **`awaitingClientAction`-чіп перевірено окремо, саме той чіп, а не сусідній alert-чіп**
    (перша спроба помилково зловила `.first()` по спільному селектору `a[href^="/orders?search="]`
    і взяла alert-чіп FR001674 — виправив вибіркою за текстом "awaiting your action"). Правильний
    чіп: `1 · awaiting your action (FR002001)` → `href="/orders?search=FR002001"` → Order List
    показує рівно один рядок `FR002001` із бейджем "Awaiting action" → Order Detail того ж номера
    теж показує бейдж. (Побічно: `Element.innerText` у Chromium повертає текст із застосованим
    CSS `text-transform: uppercase` бейджа — перша перевірка регістро-залежним `.includes()` дала
    хибний "false"; виправлено на regex без урахування регістру.)
  - **Деплой:** `vercel --prod` (не git push — так попросив користувач цього разу, на відміну від
    Етапів 4–6, де деплоїв git-інтеграції). `freitty-cabinet.vercel.app` заалайджений на новий
    білд, `/api/health` → `db:"up"`. Локальний build/tsc/eslint/test усі чисті **до** деплою.
  - `npx tsc --noEmit`, `npx eslint src`, `npm test` (81/81 — новий UI-код без нових чистих
    функцій, тестувати нічого) — усі чисті і до, і після фіксу `min-w-0`/`NeedAttentionCard`.
  / Далі: усі заплановані етапи закрито — наступний крок за користувачем. / Відкрито:
  **(1)** seed на проді не прогнаний — користувач попросив явно спитати перед цією командою, і
  сесія завершилась, не діставшись до відповіді; **(2)** зміни цієї сесії (5 файлів + новий
  `README.md`) лежать незакомічені в робочій копії — коміт не робив без явного прохання
  (`CLAUDE.md`/системні інструкції: комітити тільки за прямим запитом), хоча в усіх попередніх
  етапах закриття супроводжувалось комітом; **(3)** те саме, що й раніше і цієї сесії не
  стосується: семантика `actualQty` при `xlQty > 0`, тіло 404 без JS.
...
- `2026-09-04 14:20` — **Ревʼю Етапу 6 + виправлення всіх 13 знайдених пунктів.** Ревʼю робилось
  ...
  - **Деталь показувала менше, ніж картка, з якої на неї клікають.** `subOrders`, `destination`,
    `driverName`, `trailersCount`, `nextActionLabel` були в `OrderDetailDTO` і на `OrderCard`, але
    на сторінці деталі не рендерились взагалі. Для Consolidation це втрата суті типу. Додано
    `SubOrdersTable` (code / Ref N / pallets / alert-статус + рядок `Total pallets`) і чотири поля
    в сітку. Перевірено проти §2.7: FR001676 → `FR001676-1/REF-1001/9`, `-2/REF-1003/6`,
    `-3/REF-1002/12`, разом 27; FR001674 → `-1/REF-1005/11`, `-2/REF-1006/7` з алертом.
    `subOrdersPallets` рахується в DTO-мапері, а не в компоненті — той самий принцип, що
    `suppliesSubtotal` (B9); контракт оновлено.
  - **`alertMessage` не показувався ніде.** `hasAlert` замінює бейдж статусу (B3), тож FR001674
    давав червоний `Alert` без жодної причини на екрані — текст лежав у DTO і доїжджав тільки у
    flight-payload. Новий `OrderAlertBanner` показує причину ордера і причини сабордерів
    (`FR001674-2 · missing photo`), бо на консолідації прапорець батька зазвичай саме звідти.
  - **Екран суперечив сам собі по XL.** Рядок сітки казав `15 × Std + 3 × XL`, а картка поруч —
    `Expected (BOL) 15 · Standard`. Це не один FR001676: у сіді **10 ордерів** мають `xlQty > 0`
    разом із заповненим `actualQty` (перевірив запитом). Арифметику `computeDelta` не чіпав —
    у схемі один `actualQty` і немає XL-відповідника, тож дельта фізично може бути тільки по
    основній одиниці; натомість `ExpectedCard` тепер показує `+3 XL` поруч із числом і підпис
    `XL counted separately`, тобто картка більше не занижує BOL. Семантику `actualQty` **не**
    вигадував — див. «Відкрито».
  - **`trailerType` став справжнім полем БД** замість дописування «Trailer: Van · 53ft» у
    `warehouseNote` (рішення попередньої сесії, яке я ж і запропонував переглянути): нотатка про
    пошкоджену палету не місце для класу обладнання, а поле «Trailer» у сітці дублювало
    `TRL-8830` з «Truck / trailer». Міграція `20260904110131_trailer_type`, `schema.prisma`,
    `data-model.md` §1/§2.7 і `api-contract.md` оновлені разом — схема лишається джерелом правди.
    Для згенерованих ордерів тип **виводиться з номера трейлера, а не тягнеться з PRNG**: зайвий
    `pick()` зсунув би весь потік випадкових чисел і тихо змінив контрольні числа §2.5. Пересів:
    **16/16 assertion-ів зелені, усі 13 старих чисел без змін** (72 / 27 / 18 / 6 / 2 / 1 / 7 /
    24 / 20 / 3), плюс 3 нових на сам `trailerType`.
  - **HTTP 404 полагоджено, і причина виявилась не та, що записана раніше.** Кореневий
    `not-found.tsx` статус не лікував (його докблок це стверджував — виправив, коментар брехав).
    Справжня причина — спільний `(cabinet)/loading.tsx`: Suspense-межа над **усіма** трьома
    екранами дозволяла віддати shell зі статусом 200 ще до того, як сторінка викличе `notFound()`.
    Скелети рознесено по групах маршрутів (`(cabinet)/(overview)/`, `(cabinet)/orders/(list)/`,
    спільний `PageSkeleton`), тому `orders/[number]` більше не має межі над собою. Перевірено на
    прод-білді: `/orders/FR999999` → **404** (було 200), решта сторінок → 200, `/orders` віддає ті
    самі 6 карток, дашборд і фільтри цілі.
    **Чесне уточнення:** без JS тіло 404 усе одно не приходить у HTML — Next на `notFound()` із
    динамічного Server Component віддає error-документ, а розмітка йде стрімом і малюється на
    гідратації. Перевіряв, а не припускав: сегментний `[number]/not-found.tsx` цього не змінює
    (додав його все одно — після гідратації дає точніший текст «Order not found» і кнопку назад);
    для незіставленого URL (`/definitely-not-a-route`) тіло в HTML **є**. Тобто полагоджено рівно
    те, на що скаржились (статус-код), а залишок — обмеження фреймворку, задокументоване в коді.
  - Решта: header отримав шість контролів як у макеті (`💬 📷 $ 🖨 📦 🕘`, лічильники — реальні
    дані, клік скрізь `disabled` + `title="Out of scope"`) і кнопку «←» на `/orders`; `Status flow`
    малює всю сімку станів B4 із пройденими суцільними й непройденими приглушеними (раніше
    обрізався на поточному, через що жирний завжди був останнім і не ніс інформації) — під це
    експортовано `ORDER_PIPELINE`; додано поле `Deltas detected` (виводиться з `delta.hasDelta`,
    бо інших дельт схема не фіксує); дата в сітці тепер `2 Sep 2026, 08:00` і червона з підписом
    `· today` (нові чисті `formatDateWithYear` / `getRelativeDayLabel`); порожні фото-слоти більше
    не малюють 📷 (при `0/5` виглядало як п'ять фото), зайві згортаються в `+N`; `DeltaPanel`
    розпався на `ExpectedCard`/`ActualCard` у `DeltaCards.tsx` — фрагмент із двох карток жорстко
    прив'язував батьківський `grid-cols-3` до свого вмісту; сирі хекси Етапу 6 замінені на нові
    семантичні токени в `@theme` (amber-родина, `delta`, `chip`, `ink-soft`, `muted-soft`,
    `info-*`, `danger-*`).
  Тестів **81** (було 71): `formatDateWithYear` (включно з UTC-межею 31 Dec 23:30Z),
  `getRelativeDayLabel` (межі доби, місячна межа), `ORDER_PIPELINE` (порядок + те, що рейка
  містить кожен крок, який може повернути `getStatusFlow`). `npx tsc --noEmit`, `npx eslint
  src prisma`, `npm test`, `npm run build`, `npx prisma db seed` — усі чисті.
  / Далі: Етап 7 — Поліш (адаптив, `README.md`, перевірка з телефона). / Відкрито:
  **(1)** семантика `actualQty` при `xlQty > 0` — чи це рахунок лише основної одиниці (як його
  генерує seed), чи всього вантажу; зараз UI показує обидві частини й нічого не стверджує, але
  якщо це має бути один сумарний рахунок — треба міняти схему, а не компонент;
  **(2)** без JS тіло 404 не рендериться (вище) — обмеження Next, не баг коду.
- `2026-09-04 14:35` — **Деплой Етапів 4–6 разом.** Коміт `b49bec1` запушено в `main`; Vercel
  задеплоїв автоматично (git-інтеграція, не `vercel --prod` вручну), білд ~1 хв, статус `Ready`,
  прод-домен `freitty-cabinet.vercel.app` заалайджений на новий білд. `DATABASE_URL`/`DIRECT_URL`
  локально й на Vercel вказують на той самий Supabase, тож міграція `trailer_type` і пересів,
  зроблені під час ревʼю, вже застосовані до бази, яку читає прод — окремого кроку
  `migrate deploy` не було потрібно. Перевірено на живому URL, не тільки статусами: `/` (KPI
  7/24/3, `Need Attention` з деталлю `photo missing · FR001674` — реальні дані, не мок), `/orders`
  (6 карток), `/orders/FR001383` (`trailerType: Van · 53ft`, повна Status flow, `Deltas detected`,
  нотатка без дописаного «Trailer:»), `/orders/FR001676`, `/orders/FR001674`, `/orders/DRAFT-003`
  → 200, `/orders/FR999999` → **404** (раніше 200 — та сама знахідка ревʼю, тепер підтверджена на
  проді, не тільки на прод-білді локально), `/api/health` → `db:"up"` з реальним timestamp.
  / Далі: Етап 7 — Поліш. / Відкрито: те саме, що вище (семантика `actualQty` при XL, 404 без
  JS) — деплой їх не стосується.
- `2026-09-04 12:00` — **Етап 6 закрито.** `(cabinet)/orders/[number]/page.tsx` — Server
  Component, кличе `getOrderByNumber(number)` напряму, без `fetch` до власного
  `/api/orders/[number]` (`DECISIONS.md` A2) — жоден новий компонент не має `"use client"`,
  тож fetch з клієнта архітектурно неможливий (перевірено grep-ом на `"use client"`/`fetch(`
  по всіх нових файлах, не тільки візуально в браузері). 8 нових компонентів у
  `src/components/orders/`: `OrderDetailHeader`, `OrderInfoGrid`, `DockCard`, `DeltaPanel`,
  `WarehouseNotePanel`, `OperationsTable`, `SuppliesTable` + `DataTable` отримав опційний
  `footer` (tfoot) — потрібен для рядка "Supply subtotal", без зміни існуючих викликів.
  - **DeltaPanel** — узгоджений з користувачем план підтвердився без відхилень:
    `delta.hasDelta` (з `computeDelta` в `orders.dto.ts`, `diff !== 0`) незалежний від
    `order.hasAlert`. Перевірено вживу на трьох випадках: FR001383 (`hasAlert=false`,
    `hasDelta=true`, +2 — амбер-бейдж є, "Alert" на статусі нема), FR001674 (`hasAlert=true`
    І `hasDelta=true` водночас, diff **від'ємний** -2 — обидва бейджі показались незалежно,
    знак мінуса відрендерився коректно без `Math.abs`), FR001676 (`actualQty: null` →
    `delta.actual: null`, `hasDelta: false` — картка "Actual" нейтральна, без амбер-стилю,
    "Not weighed yet").
  - **"Van · 53ft" (відкрите питання з Етапу 1).** Обрав перший із двох варіантів користувача:
    вплів фразу в `warehouseNote` самого FR001383 в `prisma/seed.ts` ("...sent to Disposal.
    Trailer: Van · 53ft.") і пересіяв базу (13/13 assertion-ів зелені, лічильники не
    змінились) — це реальне поле з БД, нуль хардкоду на фронті. Грід-поле "Trailer" (12-те
    поле з переліку) рендерить `order.trailerNumber` (реальний ID, напр. `TRL-8830`), а не
    вигаданий тип — другий варіант користувача ("підпис поруч із trailerNumber") означав би
    статичний текст, однаковий для всіх 72 ордерів, що прямо суперечило б CLAUDE.md "нічого
    не хардкодиться". Рішення пояснене явно, не мовчки.
  - **Знахідка, варта уваги, не прихована:** `notFound()` на цій сторінці коректно рендерить
    `(cabinet)/not-found.tsx` (перевірено скриншотом — весь UI, включно з sidebar/topbar,
    правильний), але HTTP-статус відповіді лишається **200, не 404** (перевірено на
    production-білді, `curl -D -`, кількома варіантами заголовків — не артефакт dev-режиму).
    Причина: `(cabinet)/loading.tsx` створює Suspense-межу навколо сторінки, і батьківський
    `(cabinet)/layout.tsx` (який сам робить `await getCabinetUser()`) встигає почати
    стрімити shell зі статусом 200 **до того**, як асинхронна сторінка встигає викликати
    `notFound()` — задокументоване обмеження Next.js App Router (стрімінг + notFound
    несумісні, якщо над сторінкою є Suspense-межа). Це стосується **всіх** трьох екранів
    однаково (спільний layout/loading), просто Order Detail — перший екран, який взагалі
    викликає `notFound()`, тому проявилось тільки зараз. Додав кореневий `src/app/not-found.tsx`
    (раніше був відсутній — Next.js вимагає його для коректної роботи not-found-меж), але це
    **не** виправило статус-код, тільки підтвердило діагноз. Свідомо не чіпав
    `(cabinet)/layout.tsx`/`loading.tsx` — переробка спільного стрімінгу зачепила б усі три
    екрани і не була в запиті цієї сесії. Функціонально нешкідливо (правильний UI, немає
    крашу — буквально те, що просив перевірити користувач), але якщо колись знадобиться
    справжній `404` по HTTP (SEO, зовнішні перевірки) — рішення за користувачем.
  - `npx tsc --noEmit`, `npx eslint src`, `npm test` (71/71 — нових чистих функцій не
    з'явилось, Etap 6 суто UI), `npm run build` — усі чисті. Виявив і виправив побічний
    інцидент: `.next` після переключення dev↔`next start` давав `TypeError:
    routesManifest.dataRoutes is not iterable` — `rm -rf .next` перед білдом вирішило (той
    самий клас проблем, що згадувався в Етапі 3).
  / Далі: Етап 7 — Поліш (адаптив, `README.md`, прогін сіда на проді, перевірка з телефона);
  окремо варто вирішити задеплоєні етапи 4-6 разом одним деплоєм, бо прод досі на `6e7524f`.
  / Відкрито: **(1)** HTTP 200 замість 404 на `notFound()` (вище) — рішення за користувачем;
  **(2)** Етапи 4-6 сумарно ще не задеплоєні на прод; **(3)** сторінка не показує sub-orders
  консолідаційних ордерів (FR001676 має 3) — не було в явному переліку 6 елементів екрана
  цієї сесії, свідомо не додавав понад запит.
